...
class ImageNet100(ImageFolder):
    ...
    def find_classes(self, directory: str):
        ...
        classes = [d.name for d in os.scandir(directory) if d.is_dir() and d.name in self.imagenet100classes]
        classes.sort()
...
class SynImageFolder(DatasetFolder):
    ...
    def find_classes(self, directory: str):
        ...
        if self.use_imagenet == 10:
            ...
            imagenet_classes = ['American robin', 'Gila monster', 'eastern hog-nosed snake', 'garter snake', 'green mamba', 'European garden spider', 'lorikeet', 'goose', 'rock crab', 'fiddler crab', 'American lobster', 'little blue heron', 'American coot', 'Chihuahua', 'Shih Tzu', 'Papillon', 'toy terrier', 'Treeing Walker Coonhound', 'English foxhound', 'borzoi', 'Saluki', 'American Staffordshire Terrier', 'Chesapeake Bay Retriever', 'Vizsla', 'Kuvasz', 'Komondor', 'Rottweiler', 'Dobermann', 'Boxer', 'Great Dane', 'Standard Poodle', 'Mexican hairless dog (xoloitzcuintli)', 'coyote', 'African wild dog', 'red fox', 'tabby cat', 'meerkat', 'dung beetle', 'stick insect', 'leafhopper', 'hare', 'wild boar', 'gibbon', 'langur', 'ambulance', 'baluster / handrail', 'bassinet', 'boathouse', 'poke bonnet', 'bottle cap', 'car wheel', 'bell or wind chime', 'movie theater', 'cocktail shaker', 'computer keyboard', 'Dutch oven', 'football helmet', 'gas mask or respirator', 'hard disk drive', 'harmonica', 'honeycomb', 'clothes iron', 'jeans', 'lampshade', 'laptop computer', 'milk can', 'mixing bowl', 'modem', 'moped', 'graduation cap', 'mousetrap', 'obelisk', 'park bench', 'pedestal', 'pickup truck', 'pirate ship', 'purse', 'fishing casting reel', 'rocking chair', 'rotisserie', 'safety pin', 'sarong', 'balaclava ski mask', 'slide rule', 'stretcher', 'front curtain', 'throne', 'tile roof', 'tripod', 'hot tub', 'vacuum cleaner', 'window screen', 'airplane wing', 'cabbage', 'cauliflower', 'pineapple', 'carbonara', 'chocolate syrup', 'gyromitra', 'stinkhorn mushroom']
        else:
...
def get_train_val_test_dataset(dset, train_dirs: List[str], val_dir: str, test_dir: str):
    ...
    # ])
    train_datasets = SynImageFolder(train_dirs[0], use_imagenet=100)
